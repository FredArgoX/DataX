# CODE IMPLEMENTATION: Elastic Net Regression
...
from sklearn.linear_model import ElasticNet, ElasticNetCV # Elastic Net Regression
...
# ELASTIC NET REGRESSION WORKS WITH NUMERICAL DATA
...
model = ElasticNet(alpha=1.0, l1_ratio=0.5) # Manual selection of alpha value and l1_ratio (Use of ElasticNetCV recommended for finding optimal values)
...
# Model creation and  training
model_cv = ElasticNetCV(alphas=np.logspace(-6,6,100), l1_ratio=np.linspace(0.0, 1.0, 101), cv=5, random_state=36)
...
# ---> Optmal scikit-learn alpha (lambda in equation)
optimal_alpha = model_cv.alpha_
# ---> Optimal scikit-learn l1_ratio (alpha in equation)
optimal_l1_ratio = model_cv.l1_ratio_
...
print(f"{'Optimal scikit-learn alpha (lambda in equation)':<70}:{optimal_alpha:>12,.6f}")
print(f"{'Optimal scikit-learn l1_ratio (alpha in equation)':<70}:{optimal_l1_ratio:>12,.6f}")
